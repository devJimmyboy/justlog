...
export function User({ displayName, color, badges, parsed }: { displayName: string; color: string; badges: string[]; parsed: PrivateMessage }): JSX.Element {
  const renderColor = color !== '' ? color : 'grey'
  const [render, setRendering] = useState(false)
  useEffect(() => {
    if (!globalThis.badges) setTimeout(() => setRendering(true), 250)
  ...
  return (
    <UserRoot>
      {badges.map((badge, i) => {
        ...
        const bdge = globalThis.badges.find((b) => b.id === name)
        if (bdge) {
